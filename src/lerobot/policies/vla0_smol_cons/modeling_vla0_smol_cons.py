...
class VLA0SmolConsPolicy(PreTrainedPolicy):
    """Wrapper class around VLA0 model to train and run inference within LeRobot."""

    config_class = VLA0SmolConsConfig
    name = "vla0_smol_cons"

    def __init__(
        self,
        config: VLA0SmolConsConfig,
...
class VLA0(nn.Module):
    def __init__(self, config: VLA0SmolConsConfig):
        super().__init__()
        self.config = config
    ...
    def format_kinematics_prompt(self) -> str:
        if self.config.kinematics_prompt:
            return self.config.kinematics_prompt.strip()
        if not self.config.kinematics_dh_params:
            return ""
        rows = []
        for row in self.config.kinematics_dh_params:
            row_str = ", ".join(f"{value:g}" for value in row)
            rows.append(f"[{row_str}]")
        return f"{self.config.kinematics_prompt_prefix}[{', '.join(rows)}]"
    ...
    def create_prefix_tokens(
        ...
        for txt, disc_st, act in zip(lang_text, disc_states_cpu, disc_actions_cpu, strict=False):
            ...
            kinematics_prompt = self.format_kinematics_prompt()
            kinematics_segment = f", {kinematics_prompt}" if kinematics_prompt else ""
            if self.config.use_state:
                prefix = f"Task: {task_cleaned}{kinematics_segment}, State: {state_str}, Actions: "
            else:
                prefix = f"Task: {task_cleaned}{kinematics_segment}, Actions: "
